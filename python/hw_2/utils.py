...
class ColorizeMixin:
    ...
    """
    def __init__(self, repr_color_code: int = 32) -> None:
...
class AdvertBase:
    ...
    Args for init:
    - json_dict: json dict file with advertisement
    ...
    def __init__(self, json_dict: dict) -> None:
        for key, value in json_dict.items():
            ...
            setattr(self, key + '_' if keyword.iskeyword(key) else key, value)

            # if value is a dict, then make subattributes of attribute
            if type(value) == dict:
                # all functions can have arbitrary attributes, so:
